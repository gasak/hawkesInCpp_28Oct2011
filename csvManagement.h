...
//
//
//  Interface between actual data and creating a spikeTrain
//
//  Created by Siva Kumar Gorantla on 9/11/11.
//  Copyright 2011 University of Illinois. All rights reserved.
//

#ifndef hawkesInCpp_csvManagement_h
#define hawkesInCpp_csvManagement_h
...
class csvManagement {
public:    
    SubSpikeStruct csvToSpikeTrain(const char * fileName);  // read all data
    SubSpikeStruct csvToSpikeTrain(const char * fileName, int numTypes, vector<string> eventTypePtr ); // filter some 
    
private:
    double string_to_double( const std::string& s ); // helper function
};
...
SubSpikeStruct csvManagement::csvToSpikeTrain(const char * fileName, int numTypes, vector<string> eventTypes ){
	...
	while (getline (inFile, line))
	{
		...
		double time = hour_d*3600+minute_d*60+seconds_d+frac; cout << frac;
        s.setTime(time);
...
SubSpikeStruct csvManagement::csvToSpikeTrain(const char * fileName){
	...
	while (getline (inFile, line))
	{
		...
		getline (linestream, second, ',' );seconds_d = string_to_double(second); //s.setSeconds(string_to_double(second));
		getline (linestream, item, ',' ); frac = string_to_double(item);//s.setFractionOfSec(string_to_double(item));
		float time = hour_d*3600+minute_d*60+seconds_d+frac; //cout << frac << "," << time << endl;
        s.setTime(time);
        ...
        if(!eventTypesContainsElement){
            ss.numTypes = ss.numTypes+1; ss.eventTypes.push_back(item); typeIndex = ss.numTypes-1;
    ...
    for(int iter = 0; iter < ss.size; iter++){
        spike* sptr = &ss.actualData[iter]; int typeIndex = ss.getIndexOfType(sptr->getType());
        ss.entireList.push_back(sptr);
        ss.individualLists[typeIndex].push_back(sptr);
